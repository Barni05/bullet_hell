...
$(document).ready(function () {
    $("#page_size").change(function () { //Changes the page size to the currently set value
        $.ajax({
            type: "POST",
...
function GetLeaderboardData() {
    ...
    $.ajax({ //Gets the number of pages needed to display all the players n per page
        type: "GET",
    ...
    $.ajax({ //This gets all the data and puts it in a Player array 'leaderboard'
        type: "GET",
        ...
        leaderboard.forEach((player) => {
            let placementDesign;
            switch (player.rank) { //Gives the first three places different styling
                case 1:
            ...
            //The currently logged in player also gets different styling, so that its more visible
            let playerData = `<tr`;
            if (player.username == current_player) {
                playerData += ` class="current-player"`;
            }
            //Loading the table with html
            playerData += `>
        ...
        //The page controls buttons are also getting added dynamically
        var pagecontrols = "";
